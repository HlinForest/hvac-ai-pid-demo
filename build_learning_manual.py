...
OUT = Path(os.environ.get("HVAC_BATCH_DIR", str(ROOT / "archive/outputs_review_v3")))
...
def build() -> Path:
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_run(p.add_run(f"实验版本：archive/outputs_review_v3  |  生成日期：{date.today().isoformat()}"), size=9.5, color=MUTED)
    doc.add_page_break()
    ...
    add_formula(doc, 'python main.py --train-samples 48 --test-samples 16 --bo-iterations 4 --seed 23 --output archive/outputs_review_v3')
    add_formula(doc, 'python embedded/export_policy.py archive/outputs_review_v3')
