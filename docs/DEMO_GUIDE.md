...
- **`outputs_review_v3/engineering_report.html`** — 主报告，浏览器直接打开，自带图表、可打印成 PDF
- 各算法指标 CSV、训练历史、对比图

注意：`python render_report.py outputs` 只会根据**现有** CSV 重渲染报告，不会重新训练。要先跑过第 1 步。

## 第 2 步：生成离线交互演示（演示主角）

```powershell
python run_embedded_demo.py --algorithm all --provider replay
```

在 `outputs_embedded_demo/` 生成：
...
只想跑单个算法时：`python run_embedded_demo.py --algorithm zn`（可选 zn/imc/bo/safe-bo/fnn/rl/llm）。
...
1. **PPT 开场**：`outputs_review_v3/submission_package/HVAC_AI_PID_关键展示.pptx`（另有"含MCU验证""整定训练完整版"变体）
2. **交互演示**：Streamlit 网页，或双击 `outputs_embedded_demo/temperature_control_demo.html`
3. **技术细节**：`outputs_review_v3/engineering_report.html`（含三层交叉验证证据链）
4. **数据表格**：`outputs_review_v3/submission_package/HVAC_AI_PID_仿真实验结果.xlsx`
...
| LLM 自动整定（阿里云百炼） | 把 key 写入项目根目录 `.env`（模板见 `.env.example`，或 `$env:DASHSCOPE_API_KEY="..."`），然后 `python run_llm_agent_demo.py --provider openai-compatible --model qwen-plus` | 百炼 API key，产生费用 |
...
| LLM 自动整定（OpenAI 官方） | `$env:OPENAI_API_KEY="..."` 然后 `python run_llm_agent_demo.py --provider openai --model "<模型名>"` | OpenAI API key，产生费用 |
| LLM 自动整定（本地模型） | `python run_llm_agent_demo.py --provider ollama --model "<支持工具调用的模型>"` | 本地安装 Ollama |
...
1. `outputs_embedded_demo\temperature_control_demo.html`（现场交互）
