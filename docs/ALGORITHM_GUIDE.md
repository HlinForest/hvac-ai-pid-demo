...
**为什么是 Value-Based 的表格式 Q 学习，而不是 Policy-Based？** 本任务动作只有 9 档、状态 75 格，一张 675 项的 Q 表足够装下：可整表审计、可直接上 MCU（88 µs 查表）、"先屏蔽不安全动作再 argmax"保证策略永不越界，"哪个格子没见过"（覆盖掩码与 IMC 回退）也是表格法特有的概念。策略网络适合连续动作与高维状态，对本问题是黑箱且需重做全部安全层。若未来需要连续增益目标，建议优先加密动作网格仍保持表格式。
...
```text
error bin(e, 5 档):     边界 [-2.0, -0.5, +0.5, +2.0] °C（严重偏冷…严重偏热）
delta bin(ė, 5 档):     边界 [-0.15, -0.03, +0.03, +0.15] °C/min（快速变冷…快速变热）
command bin(u_prev, 3 档): 边界 [0.05, 0.70]（停机 / 部分负荷 / 高负荷）
thermal_state = bin(e) × bin(ė) = 25 个热状态
action = 相对 IMC 的绝对 Kp 目标比例 × Ki 目标比例 = 9 个动作
  (Kp, Ki) 目标 ∈ {0.75, 1.00, 1.30}× 的 3×3 组合，动作 4 = (1.00, 1.00) 即"保持 IMC"
Q-table shape = 5 × 5 × 3 × 9
```

动作还有按误差档位的安全屏蔽：偏冷（档 0–1）只允许 Kp、Ki 均不升的 4 档；带内（档 2）屏蔽升 Ki；偏热（档 3–4）放开全部 9 档——探索动作同样只能在白名单内随机。
...
超参数取值（v3 封存运行固定未再调优）：**α=0.12**——奖励混有场景随机性、传感器噪声（σ=0.04 °C）与探索动作，小步长相当于对每个格子约 8 次近期更新做平均，防止单条噪声改写整表；**γ=0.94**——有效视野 ≈ 1/(1−γ) ≈ 17 次决策 ≈ 83 min，刻意短于 240 min 回合，让 Q 只回答"近期局部改善"，回避长程 credit assignment；**ε 由 0.25 线性退火至 0.03**——先覆盖后收敛。一次完整 rollout 走查（微型地图 + 逐步表格，取自封存 `rl_training_transitions.csv` 第 1 回合）见 `reports/实验报告_AI-PI整定.md` §4.8.3 与分报告第 09 篇。
