...
- **`outputs/engineering_report.html`** — 主报告，浏览器直接打开，自带图表、可打印成 PDF
- 各算法指标 CSV、训练历史、对比图
...
1. **PPT 开场**：`outputs/submission_package/HVAC_AI_PID_关键展示.pptx`（另有"含MCU验证""整定训练完整版"变体）
2. **交互演示**：Streamlit 网页，或双击 `outputs_embedded_demo/temperature_control_demo.html`
3. **技术细节**：`outputs/engineering_report.html`（含三层交叉验证证据链）
4. **数据表格**：`outputs/submission_package/HVAC_AI_PID_仿真实验结果.xlsx`
