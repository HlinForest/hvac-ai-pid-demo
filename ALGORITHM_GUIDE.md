...
`λ` 越大，控制越稳但恢复越慢。代码将 `λ` 至少设成延迟的三倍，因此 IMC 是本工程的**保守安全基线和故障回退控制器**。这也是为何部分场景下 IMC 的 ITAE 可能较大，但压缩机输出方差较低。
...
- 输入 1：`e`，中心点为 `[-5, -2.5, 0, 2.5, 5] °C`；
- 输入 2：`Δe`，中心点为 `[-1, -0.5, 0, 0.5, 1] °C/采样`；
...
```text
state = bin(e, 5 档) × bin(Δe, 5 档) = 25 个状态
action = ΔKp 比例 × ΔKi 比例 = 9 个动作
Q-table shape = 5 × 5 × 9
```

训练函数在同一个 **3R2C 房间 + 延迟/惯性压缩机** 虚拟环境中执行 500 个 episode。每一步用 epsilon-greedy 选动作，动作只会给 `Kp/Ki` 一个很小的相对变化；热模型推进到下一步后，再以温差、动作幅度和压缩机指令计算奖励：

```text
r = -|下一步误差| - 0.18·|增益改变量|
    - 0.04·|Kp比例偏离1| - 0.03·PWM
...
`IncrementalRLController` 只需：状态分箱 → `argmax(Q[state])` → 取一个相对增益增量 → 走同一安全屏蔽层。它不直接输出 PWM，因此即使策略有误，也会受到 PI 饱和、增益上下限、25% 变化率以及 IMC 回退共同约束。
...
- `compressor_output_variance` 小：压缩机指令更平稳，潜在机械磨损与频繁调速更低；
- `fallback_events` 非零：说明自适应控制触发了保护，应调查异常输入或策略边界。

不能只按单一目标函数排名。精密/机柜空调更应同时看稳定率、ITAE、扰动恢复和 PWM 方差。
