...
- **同一控制架构**：100 ms PI 内环（条件积分抗饱和）+ 三口径AI调度层（Python仿真300 s / 演示120 s / MCU墙钟2 s，详见 hvac_pid/timebase.py）；AI 只调 Kp/Ki，不直接控制压缩机；四层安全保护（节拍解耦、增益界、单次变化 ≤10%、异常回退 IMC）。
...
- **诚信标注**：结论限于仿真与 SIL 层；安全 BO / LLM 的三标准工况与混合工况曲线为**冻结增益补跑**（标注"补跑"，与 v3 同对象、同约束、同种子）；LLM 矩阵 v3（outputs_llm_matrix_v3）18 次真实调用、5/18 部署；v4 复现矩阵（artifacts/runs/v4-20260906-8ac9bf4/llm_matrix_v4）18 次真实调用、1/18 部署，另 3/18 fallback 自身不稳定；06 走读已换 v4 会话（qwen-max_std_r72）；输出非确定性需报告多次运行分布与回退率。
...
- **自整定（self-tuning）**：投运后控制器运行中在线修正自身参数（本项目：FNN / RL，Python仿真300 s / 演示120 s / MCU墙钟2 s三口径分离）。
...
| 报告 | 配图文件（新图在 `reports/分报告/figures/`；22 张复用图单源于 `reports/figures/`，正文经 `../figures/` 引用） | 内容 |
|---|---|---|
...
| `outputs_review_v3/submission_package/` | **历史交付包**：2R2C 对象、6 留出工况、20 训练工况（依据为该目录 `source-notes.txt` 的自述） | **否**——数值口径与本套报告不一致，不可混用 |
| `outputs/`（可重跑 regenerable scratch，封存以 `outputs_review_v3/` 为准） | 主实验产物目录 | 部分——仅 01 引用的 24 h 交叉验证时序 |
