...
先做阶跃辨识，再把 K、τ、L 代入 Z-N 公式，最后检查激进参数的代价。
...
在没有 AI 之前，工程师怎么调 PI？最老的做法是：做一次阶跃试验，拟合出三个数，代入经验公式，参数就出来了，全程不到 1 秒。它是本项目在 E2 密封谱内的对照基线——AI 方法先证明在本谱内优于它。
...
- 符号与单位：K（°C/指令）、τ、L、Ti 都是 min；Kp 单位为容量比例/°C，Ki 为容量比例/(°C·min)；输入全部来自辨识，没有搜索。
