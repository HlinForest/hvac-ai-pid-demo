...
先理解 λ 怎样改变快慢，再扫描候选、冻结 IMC 基线并检查稳定性。
...
- 符号与单位：K（°C/指令）来自辨识温降换算，τ、L、λ、Ti 都是 min；Kp 单位为容量比例/°C（无量纲指令每 °C 误差），Ki 为容量比例/(°C·min)。
...
> **这说明什么**：钝而稳——在三工况与训练谱内未见失稳，也不会针对当前工况加速。这正是它被选为全部 AI 方法备用基准的原因（备用≠各场景验收通过，见第 11 篇验收率）。
...
- 未证明的部分：λ 按平均目标选，不保证最坏工况最优；单组固定增益在本谱内的经验上限大抵如此，要突破得换思路（第 06 篇让程序搜，第 08、09 篇让参数动起来）。
