...
- 当前最终训练访问了25/25个(e,ė)热状态，以及47/75个“热状态×实际容量模式”组合；未访问组合保持IMC回退并点亮回退灯。覆盖只证明数据到达，不证明策略优于基线，是否部署由验证目标和验收标志共同决定。

在线实跑状态（2026-08-23）：ESP32 与 STM32F103 的完整算法均已通过 Wokwi 构建并进入运行态；详细证据边界见 `archive/outputs_review_v3/mcu_wokwi_validation.csv`。目前尚未取得可复核的串口曲线、目标 ROM/RAM 与最坏周期，因此仍需按 `embedded/README.md` 的清单补齐最终验收。
