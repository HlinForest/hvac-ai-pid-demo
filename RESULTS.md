...
## E2-B3：独立密封重评（当前主实验；各方法按既定训练配置重新冻结后的统一场景评价）
...
| LLM replay | 41.24 | 1.0000 与 IMC 持平 | 40/80 (50.0%) | 完整 48 训练集冻结仍无接受候选，等于 IMC；非实时调用 |
  ...
  - E3+ 严格对照（`gp_attribution.csv` 12 行 + `gp_attribution_summary.json`；同初值同噪声，4 搜索种子）：
    混合池配对差 GP−随机 −0.0057 [−0.0373,+0.0284]（含 0，无优势证据），GP−初值 −0.1115 [−0.1419,−0.0791]（追加评估优于不再评估）。
    读法：**未发现 GP 相对同预算随机的优势，机制贡献未经证实**；逐种子波动大（9002 种子纯随机反超 GP），单次比较不可作机制结论。
  - 失败归因（`failure_cause_summary.csv`）：7 个场景全算法失败（1 容量紧张 + 6 观察窗不足，属实验设计/物理边界）；
    IMC 系 33 个调参残余失败（全部含 `recovery_ok` 未过，29 个兼 `comfort_held` 未过；负荷比 ≤0.62、窗口 ≥1 h → 真实控制短板在扰动恢复）。
    无任何方法系统性挽救 IMC 失败场景（RL 挽救 #36 但新增 #25/#37；BO 新增 7 个失败，挽救 0）。
    J 方向一致：失败均值 ~55 vs 通过均值 ~27（约 2×），目标与验收无系统性冲突。
